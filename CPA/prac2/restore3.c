...
void swap( Byte a1[],Byte a2[],int rw,int rh,int w ) {
  ...
  if ( a1 != a2 ) {
    ...
    #pragma omp parallel
    { 
    	#pragma omp for private(x,d,aux) schedule(runtime)
	    for ( y = 0 ; y < rh ; y++ ) {
...
void process( int w,int h,Byte a[], int bw,int bh ) {
  int x,y, x2,y2, mx,my,min, d, niter, prMax, prMin,id;
  ...
  for ( y = bh ; y < h ; y += bh ) {
    ...
    #pragma omp parallel private(prMax,prMin,id,niter)
    {
      prMin = INT_MAX; prMax = 0; niter = 0; id = omp_get_thread_num();
...
int main(int argc,char *argv[]) {
  ...
  printf("El número de hilos que se ejecutan en paralelo es %d\n", nprocs);
