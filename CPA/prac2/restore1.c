...
int distance( int n, Byte a1[], Byte a2[], int stride ) {
  ...
  #pragma omp parallel 
  {
  	#pragma omp for private(j,r,g,b) reduction(+:d) schedule(runtime)
	  for ( i = 0 ; i < n ; i++ ) {
	    j = i * stride;
...
int main(int argc,char *argv[]) {
  ...
  //Muestra el nº de hilos (no dentro de process al llamarse varias veces)
  printf("El número de hilos que ejecutan el método process es: %d\n", nprocs);
